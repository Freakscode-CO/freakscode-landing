...
const SurveyForm: React.FC<SurveyFormProps> = ({ survey }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await submitSurvey(survey.id, formData); // Use imported submit function
    ...
    const checkDependency = (dependsOn?: Survey['blocks'][0]['questions'][0]['dependsOn']): boolean => {
        if (!dependsOn) {
        ...
        if (condition === 'includes') {
            // Check if the array of values for a multi-select includes the target value
            return Array.isArray(dependentValue) && dependentValue.includes(value);
        }

        // Default check: compare single value
    ...
    // Luego verificamos la pantalla de contexto
    if (showContext) {
        return (
            <div className="max-w-2xl mx-auto p-8 bg-white rounded-xl shadow-lg border-t-4 border-indigo-600 transform transition-all duration-500 hover:scale-[1.02]">
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-2xl font-bold text-indigo-900 mb-4">{survey.title}</h2> {/* Use survey title */}
                </div>
                <p className="text-gray-700 mb-8 text-lg leading-relaxed text-center">
                    {survey.description} {/* Use survey description */}
                </p>
    ...
    // Por último, verificamos el estado intermedio
    if (showIntermediate) {
        return (
    ...
    const currentBlock = survey.blocks[currentStep - 1]; // Get current block data (0-based index)
                ...
                {survey.blocks.map((block, index) => (
                    ...
                    >
                        ...
                        <div className="space-y-4">
                            {/* Render questions using the subcomponent */}
